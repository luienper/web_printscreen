...
ListView.include({
	...
	export_to_excel: function(export_type) {
            ...
            // Find Header Element
            var header_eles = self.$el.find('thead')
...
//	                        text = instance.web.parse_value(text, { type:"float" })
            ...
            if (export_type === 'excel'){
                ...
                new Model("res.users").query(['company_id']).filter([['id', '=', this.session.uid]]).
                limit(15).all().then(function (res) {
                    new Model("res.company").query(['name']).filter([['id', '=', res[0]['company_id'][0]]]).limit(15).all().then(function (result) {
